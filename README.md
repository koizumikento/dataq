...
- qsv stats CSV のデータセット件数は、明示的な `record_count|records|rows|row_count|total_rows`、または `Integer` 行で全セルが非空の `n_negative+n_zero+n_positive+nullcount` から決定し、全フィールドで同じ件数を使う。`Float` の符号別カウンタは `NaN` などを数えないため件数根拠には使わない。空セルは利用不能、リテラル `0` は有効値として扱い、文字列列を 0 件とはみなさない
