...
const PROFILE_NOTES: &[&str] = &[
    ...
    "qsv normalization requires one exact dataset `record_count` from `record_count|records|rows|row_count|total_rows`, complete signed counters on an Integer row, or provably empty metadata; Float counters can omit NaN and are not count evidence.",
    "For qsv numeric rows, `numeric_stats.count` is the checked sum of complete `n_negative+n_zero+n_positive` cells; incomplete counters omit the stats, and Float NaN rows do not inflate the numeric sample count.",
    "qsv count cells require canonical nonnegative decimal integers; fractions, exponent notation, negatives, and out-of-range values are input errors.",
