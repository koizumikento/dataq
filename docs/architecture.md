...
また、`jq` / `yq` / `mlr` / `xh` の組み合わせを運用向けに単純化して再利用するためのオーケストレーション層を担います。

## 基本方針

- 実装本体は Rust（ネイティブCLI）
- 依存コマンドは `jq`, `yq`, `mlr`（Miller）, `xh` を前提とする
- デフォルト出力は JSON（機械可読）
- `stdin -> stdout` を第一に設計
- キー順、時刻フォーマット、丸め規則を固定して決定性を担保
- Rust層が外部ツール実行をオーケストレーションし、I/O契約と終了コードを統一する
- 必要な機能では `jq` / `yq` / `mlr` の多段連携（例: `yq -> jq -> mlr`）を内部実行し、3ツールの合わせ技を1コマンドへ固定する
...
- `adapters`: 外部ツール呼び出し（`jq`/`yq`/`mlr`/`xh`）
...
- ただし、外部ツール多段連携そのものが価値の中心である機能は、`adapters` を第一の実行経路として設計する
...
- `assert --normalize` は `yq -> jq -> mlr` の固定3段で実行し、段順を決定性契約として扱う
- `ingest yaml-jobs` は `yq -> jq -> mlr` の固定3段でジョブレコードを生成し、`assert` など上流入力として再利用可能にする
