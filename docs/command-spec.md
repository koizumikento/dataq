...
- 実行方式:
  ...
  - `--emit-pipeline` で `stage_diagnostics` に `join_mlr_execute` を出力（`input_records`, `output_records`, `input_bytes`, `output_bytes`, `duration_ms`, `status`）
...
- 実行方式:
  - `mlr` を明示的引数配列で実行（シェル展開なし）
  - `--emit-pipeline` で `stage_diagnostics` に `aggregate_mlr_execute` を出力（`input_records`, `output_records`, `input_bytes`, `output_bytes`, `duration_ms`, `status`）
...
- `stage_diagnostics` (optional): 段ごとの診断情報（`order`, `step`, `tool`, `input_records`, `output_records`, `status`）
  - 追加メトリクス: `input_bytes`, `output_bytes`, `duration_ms`
